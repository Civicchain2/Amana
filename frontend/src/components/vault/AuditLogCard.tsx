...
const logIcons: Record<LogType, ReactNode> = {
  biometric: <Fingerprint className="w-5 h-5 text-emerald" />,
  "multi-sig": <Radio className="w-5 h-5 text-gold" />,
  ledger: <Database className="w-5 h-5 text-text-secondary" />,
};
      ...
      <div className="space-y-4">
        {entries.map((entry, index) => (
          <div key={index} className="flex items-start gap-3">
            <div
              className={`w-10 h-10 rounded-full ${logBgColors[entry.type]} flex items-center justify-center flex-shrink-0`}
            >
              {logIcons[entry.type]}
            </div>
            <div>
